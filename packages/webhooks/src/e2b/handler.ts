...
export async function handleE2bWebhook(
	...
	if (webhookSecret) {
		if (!signatureHeader) {
			return { success: false, eventId: null, error: "Missing e2b-signature header" };
		}

		const valid = verifyE2bSignature(webhookSecret, rawBody, signatureHeader);
		if (!valid) {
			return { success: false, eventId: null, error: "Invalid webhook signature" };
		}
	}
	...
	try {
		...
		return {
			success: false,
			eventId: null,
			error: "DB insert failed: " + (error instanceof Error ? error.message : ""),
		};
	}
	...
	if (newStatus) {
		...
		if (findProject) {
			await db
				.update(project)
				.set({ sandboxStatus: newStatus, updatedAt: new Date() })
				.where(eq(project.sandboxId, payload.sandbox_id));
		}
	}
