...
/**
 * Scaffold a new tRPC procedure in @acme/api with the correct boilerplate.
 * Produces the exact code skeleton the agent should write, reducing
 * hallucinated router patterns.
 */
export const scaffoldTrpcProcedure = tool(
	async ({
		routerName,
		procedureName,
		procedureType,
		...
		const procedure = requiresAuth ? "protectedProcedure" : "publicProcedure";
		const template = `
// Add to packages/api/src/routers/${routerName}.ts

${procedureName}: ${procedure}
  .input(${inputSchema})
  .${procedureType}(async ({ input${requiresAuth ? ", ctx" : ""} }) => {
    ${requiresAuth ? "const { user } = ctx.session;" : ""}
    // TODO: implement ${procedureName}
    throw new TRPCError({ code: "NOT_IMPLEMENTED" });
  }),`.trim();

		return JSON.stringify({ template, file: `packages/api/src/routers/${routerName}.ts` });
	},
	{
		name: "scaffold_trpc_procedure",
		description:
			"Generate a tRPC v11 procedure skeleton for @acme/api. " +
			"Use this before writing a new API endpoint to get the correct boilerplate. " +
			"The result is the exact code to paste into the router file.",
		schema: z.object({
			routerName: z.string().describe("Router file name without extension, e.g. 'agent' or 'project'"),
			procedureName: z.string().describe("Procedure name, e.g. 'create' or 'listSessions'"),
			procedureType: z.enum(["query", "mutation", "subscription"]),
			inputSchema: z.string().describe("Zod schema expression as a string, e.g. 'z.object({ id: z.string().uuid() })'"),
			requiresAuth: z.boolean().describe("Whether to use protectedProcedure (true) or publicProcedure (false)"),
		}),
	}
);

/**
 * Scaffold a new Drizzle ORM table definition.
 */
export const scaffoldDrizzleTable = tool(
	async ({
		tableName,
		columns,
		hasTimestamps,
		...
		const cols = columns
			.map((c) => {
				let col = `  ${c.name}: ${c.type}("${c.name}")`;
				if (!c.nullable) col += ".notNull()";
		...
		const template = `
// Add to packages/db/src/schema/${tableName}.ts
...
export type ${capitalize(tableName)}    = InferSelectModel<typeof ${tableName}>;
export type New${capitalize(tableName)} = InferInsertModel<typeof ${tableName}>;`.trim();

		return JSON.stringify({
			template,
			file: `packages/db/src/schema/${tableName}.ts`,
			nextStep: "Run pnpm db:generate after editing the schema file",
		});
	},
	{
		name: "scaffold_drizzle_table",
		description:
			"Generate a Drizzle ORM table definition for @acme/db. " +
			"Use this before creating a new database table. " +
			"After writing the file, instruct the supervisor to run pnpm db:generate.",
		schema: z.object({
			tableName: z.string().describe("Snake_case table name, e.g. 'agent_session'"),
			columns: z.array(
				z.object({
					name: z.string().describe("Column name in snake_case"),
					type: z.string().describe("Drizzle column type call, e.g. 'text' or 'uuid'"),
					nullable: z.boolean().optional().describe("Whether the column is nullable (default: false = NOT NULL)"),
					default: z.string().optional().describe("Default value expression, e.g. '\"user\"' or 'false'"),
				})
			).describe("Array of column definitions"),
			hasTimestamps: z.boolean().describe("Add createdAt and updatedAt timestamp columns"),
		}),
	}
);
