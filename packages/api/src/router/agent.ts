    ...
    .subscription(async function* ({ input, ctx, signal }) {
      ...
      if (lastEventId !== undefined) {
        ...
        for (const row of missed) {
          yield tracked(String(row.seq), row.data as unknown as AgentEvent);
        }
      }
      ...
      const streamResult: TransformResult = { interrupted: false };
      ...
      try {
        ...
        for await (const agentEvent of transformAgentStream(
          rawStream,
          streamResult,
        )) {
          seq++;
        ...
        if (streamResult.interrupted) {
          // Agent was paused for HITL — don't mark done or close sandbox.
          // The client should call resolveHitl, then agent.run again.
          await db
            .update(agentSession)
            .set({ status: "paused", updatedAt: new Date() })
            .where(eq(agentSession.id, sessionId));
